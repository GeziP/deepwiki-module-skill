...
1. ASCII diagrams properly formatted (MD) / ASCII 图表格式正确
2. SVG diagrams properly rendered (HTML) / SVG 图表正确渲染
3. Code examples runnable / 代码示例可运行
4. Cross-reference links present / 交叉引用链接存在

### Phase 5: Mermaid Validation (if applicable) / Mermaid 校验（如适用）

If the generated HTML contains Mermaid code blocks (e.g., via md-to-html.js conversion), run validation:
如果生成的 HTML 包含 Mermaid 代码块（如通过 md-to-html.js 转换），需执行校验：

```bash
node -e "
const fs = require('fs');
const f = 'doc/tech-docs/<ModuleName>_Design.html';
