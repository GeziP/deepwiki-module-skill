#!/usr/bin/env node
/**
 * validate-html.js — DeepWiki HTML 闭环校验脚本 (模块级文档版)
 *
 * 8 类校验，每类闭环：检查 → 自动修复(可选) → 重新检查 → 报告
 *
 * Usage:
 *   node validate-html.js doc/tech-docs/Task_Design.html
 *   node validate-html.js --all              # 校验 doc/tech-docs/ 下所有模块设计 HTML
...
if (!convertAll && files.length === 0) {
  ...
  console.log('  --all      Validate all HTML in doc/tech-docs/');
...
function checkHtmlSkeleton(html, report) {
  ...
  for (const c of checks) {
    if (!c.test.test(html)) missing.push(c.name);
  }
